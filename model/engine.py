...
def _is_degenerate_formula(formula: list[int]) -> tuple[bool, str]:
    """检测公式是否退化（2026-07-20 新增，2026-07-20 扩展）。

    退化模式包括：
    1. DIV(X, X) — 相同值相除恒等于1，是无效操作
    2. SUB(X, X) — 相同值相减恒等于0，是无效操作
    3. IF_GT(X, X, X) — 三个操作数相同，条件无意义
    4. IF_GT(X, Y, Y) — 后两个分支相同，等价于直接返回Y
    5. 连续多个相同特征堆叠后只做恒等运算
    6. 重复一元算子链 — 如 CLIP(CLIP(CLIP(RET)))，幂等算子多次应用无意义
    ...
    # 构建算子 arity 映射
    op_arity = {}
    for idx, (name, _, arity) in enumerate(OPS_CONFIG):
        token = feat_offset + idx
        op_arity[token] = arity

    # ── 预扫描：检测重复一元算子链 ──────────────────────────────────
    # 如 [0, 123, 123, 123, 123, 123, 123, 123] = CLIP(CLIP(CLIP(...RET)))
    # 一元算子连续出现 3+ 次几乎必然退化（幂等或近幂等）
    consecutive_count = 1
    for i in range(1, len(formula)):
        t = int(formula[i])
        prev_t = int(formula[i - 1])
        if t == prev_t and t in op_arity and op_arity[t] == 1:
            consecutive_count += 1
            if consecutive_count >= 3:
                op_name = op_names[t] if t < len(op_names) else f"OP{t}"
                return True, (
                    f"{op_name}连续{consecutive_count}次："
                    f"token {i} 处重复一元算子链，幂等操作无意义"
                )
        else:
            consecutive_count = 1

    # 模拟栈，追踪每个栈元素的来源（用 token 序列标记）
    stack: list[list[int]] = []
    ...
    for i, token in enumerate(formula):
        ...
        if token < feat_offset:
            ...
            if len(stack) < arity:
                return False, ""  # 栈不足，非法公式，不判定
            args = stack[-arity:]
            # 弹出
            stack = stack[:-arity]

            op_name = op_names[token] if token < len(op_names) else f"OP{token}"

            # 检测退化模式
            if op_name == "DIV" and arity == 2:
                # DIV(X, X) — 两个操作数来源完全相同
                if args[0] == args[1]:
                    return True, f"DIV(X,X) 退化：token {i} 处相同值相除恒等于1"

            elif op_name == "SUB" and arity == 2:
                # SUB(X, X) — 两个操作数来源完全相同
                if args[0] == args[1]:
                    return True, f"SUB(X,X) 退化：token {i} 处相同值相减恒等于0"

            elif op_name == "IF_GT" and arity == 3:
                # IF_GT(X, X, Z) — 条件和then分支相同
                if args[0] == args[1]:
                    return True, f"IF_GT(X,X,Z) 退化：条件与then分支相同"
                # IF_GT(X, Y, Y) — then和else分支相同
                if args[1] == args[2]:
                    return True, f"IF_GT(X,Y,Y) 退化：then与else分支相同，条件无意义"
...
class AlphaEngine:
    def __init__(self, data_manager=None, use_lord_regularization=True,
        ...
        self.timeframe     = timeframe       # K 线周期（如 '1H', '5m'），用于策略文件命名
        self.data_file     = data_file       # 训练数据文件路径，写入策略 JSON 元数据
        ...
        if data_manager is not None:
            try:
                ...
                self.bt.periods_per_year = infer_periods_per_year(
                    data_manager.raw_dict.get("time"),
                    default=self.bt.periods_per_year,
                )
            except Exception:
                pass
            # 若未显式传入 timeframe，从 raw_dict 推断（从文件名）
            if self.timeframe is None and data_manager is not None:
                try:
                    from data_pipeline.parquet_manager import inspect_parquet_file
                    _raw = getattr(data_manager, 'raw_dict', {})
                    _fp = _raw.get('file_path') if isinstance(_raw, dict) else None
                    if _fp:
                        _info = inspect_parquet_file(_fp)
                        self.timeframe = _info.get('timeframe')
                except Exception:
                    pass
        ...
        self.stop_requested       = False   # Web UI 停止训练标志
    ...
    def _update_elite_pool(self, val_score: float, formula: list[int], step: int = 0) -> None:
        ...
        2026-07-20: 新增退化公式检测，退化公式不进入精英池。
        """
        # 退化公式检测：DIV(X,X) / SUB(X,X) / IF_GT(X,Y,Y) 等
        is_deg, _ = _is_degenerate_formula(formula)
        if is_deg:
            return
    ...
    def _apply_corr_penalty(self, reward: torch.Tensor, factor: torch.Tensor) -> torch.Tensor:
        ...
        if f_flat.std() < 1e-4:
            return reward
        # 防御性维度检查：清理与新数据维度不一致的过期因子（防崩溃）
        cur_len = f_flat.shape[0]
        if any(f.shape[-1] != factor.shape[-1] for _s, _c, f in self.factor_pool):
            self.factor_pool = [e for e in self.factor_pool
                                if e[2].shape[-1] == factor.shape[-1]]
            if not self.factor_pool:
                return reward
        pool_vecs = torch.stack(
            [f.reshape(-1).float() for _, _cnt, f in self.factor_pool], dim=0
        )
        # 二次校验：若展平后长度仍不一致（如 N 维也不同），跳过惩罚
        if pool_vecs.shape[1] != cur_len:
            return reward
    ...
    def train(self, start_step: int = 0, end_step: int | None = None,
        ...
        if verbose_header:
            ...
            print(f"   Actor-Critic: 启用={ModelConfig.USE_ACTOR_CRITIC}  "
                  f"权重={ModelConfig.CRITIC_COEFF}")
        ...
        for step in pbar:
            ...
            if self.stop_requested:
                tqdm.write(f"[停止] 用户请求停止训练 @ 第{step}步（最优={self.best_score:.3f}）")
            # 停止时也保存训练历史，确保 Web UI 曲线完整
                try:
            ...
            last_value_new = None  # P0-2: critic value from last sampling step

            for si in range(ModelConfig.MAX_FORMULA_LEN):
                lg, val, _ = self.model(inp_new)
                ...
                last_value_new = val  # P0-2: keep last step's critic value
            ...
            for i, fml in enumerate(all_fmls):
                ...
                if final_val > self.best_score:
                    # 退化公式检测（2026-07-20）：拒绝 DIV(X,X) 等退化公式登顶
                    is_deg, deg_reason = _is_degenerate_formula(fml)
                    if is_deg:
                        tqdm.write(
                            f"[退化跳过 @ 第{step}步] 验证={final_val:.3f} | {deg_reason}"
                        )
                        pass
                    else:
            ...
            # P0-2: Actor-Critic — 用 head_critic 的逐公式 value 估计做 baseline
            batch_mean = rewards.mean().item()
            batch_std  = rewards.std().clamp(min=0.1)

            # Critic loss（始终计算，让 critic 在 warmup 期间也学习）
            critic_loss = torch.zeros(1, device=ModelConfig.DEVICE)
            if (ModelConfig.USE_ACTOR_CRITIC and last_value_new is not None
                    and last_value_new.numel() > 0):
                critic_target = rewards[:n_new].detach()
                critic_loss = F.mse_loss(last_value_new.squeeze(-1), critic_target)

            # Advantage: critic baseline（warmup 后）或 EMA/batch mean（warmup 期间）
            use_critic_baseline = (
                ModelConfig.USE_ACTOR_CRITIC
                and last_value_new is not None
                and last_value_new.numel() > 0
                and self._reward_ema_step >= ModelConfig.REWARD_EMA_WARMUP
            )
            if use_critic_baseline:
                critic_vals = last_value_new.squeeze(-1).detach()  # [n_new]
                adv = torch.zeros_like(rewards)
                adv[:n_new] = (rewards[:n_new] - critic_vals) / (batch_std + 1e-5)
                if n_elite > 0:
                    ema_base = self._reward_ema if self._reward_ema is not None else batch_mean
                    adv[n_new:] = (rewards[n_new:] - ema_base) / (batch_std + 1e-5)
            elif ModelConfig.REWARD_EMA_BASELINE and self._reward_ema_step >= ModelConfig.REWARD_EMA_WARMUP:
                baseline = self._reward_ema
                adv = (rewards - baseline) / (batch_std + 1e-5)
            else:
                adv = (rewards - batch_mean) / (batch_std + 1e-5)

            # 更新 EMA（保留作为 critic 的 fallback）
            ...
            loss = policy_loss - ent_coeff * mean_ent + ent_floor_loss \
                   + ModelConfig.CRITIC_COEFF * critic_loss
        ...
        # ── End of training ──────────────────────────────────────────
        # 训练结束（跑满或被停止）都保存最终 strategy 和历史
        # 注意：_save_strategy_live() 已在每次发现新最优时即时保存，
        # 这里是最终一致性保存，确保 strategy JSON 和 history 文件是最新的。
        save_path = None
        if self.best_formula is not None:
            from .vocab import VOCAB_VERSION
            strategy_data = {
        ...
        status_msg = "手动停止" if self.stop_requested else "训练完成"
        print(f"\n[{status_msg}] {sym_tag}训练结束！")
    ...
    def _save_training_history_live(self) -> None:
        ...
        try:
            # 历史文件名包含 timeframe，避免不同周期训练历史互相覆盖
            parts = []
            if self.target_symbol:
                parts.append(self.target_symbol)
            if self.timeframe:
    ...
    def _save_strategy_live(self) -> None:
        ...
        try:
            ...
            strategy_data = {
                "vocab_version": VOCAB_VERSION,
                "symbol": self.target_symbol,
                "timeframe": self.timeframe or existing.get("timeframe"),
                "data_file": self.data_file or existing.get("data_file"),
                "formula": self.best_formula,
                "best_score": self.best_score,
                "formula_decoded": self._decode_formula(self.best_formula),
            }
            # 保留训练数据路径等元数据，避免 live 保存把 data_file 冲掉
            for key in ("mode", "train_steps"):
