...
class MT5Backtest:
    ...
    def __init__(
        self,
        cost_rate:        float = 0.0008,   # 手续费+滑点：0.0005+0.0003
    ...
    def _cost_stress(
        ...
        成本率已含手续费+滑点（0.0008），stress_mult=2 → 压力测试成本 0.0016。
    ...
    def _turnover_penalty(self, turnover: Tensor) -> Tensor:
        """梯度式换手率惩罚（2026-07-20 加强）。

        原参数阈值 0.2 过高，高频策略在 0.15 时就开始获利。
        新参数：阈值 0.15，系数 5.0，上限 5.0，更早更重地惩罚高频换手。
        """
    ...
    def _multi_objective(
        ...
        if N == 1:
            ...
            if ModelConfig.REWARD_MODE == "ftmo":
                # FTMO 专属（2026-07-20 调整）：
                #   年化收益 0.80→0.40：原权重过高导致模型偏好高频退化策略
                #   Sortino 0.05→0.15：风险调整收益提权
                #   Calmar 0.10→0.15：回撤控制提权
                #   IC 0.03→0.15：信号质量提权，抑制无alpha公式
                #   tq 0.02→0.15：交易频率质量提权，惩罚过度换手
                return (
                    0.40 * ann_ret           # 主目标：年化绝对收益（降权，防高频刷分）
                    + 0.15 * port_sortino    # 风险调整辅助（提权）
                    + 0.15 * port_calmar     # 回撤控制（提权，对齐 10% Max Loss）
                    + 0.15 * ts_ic           # IC 预测方向（大幅提权，抑制无alpha公式）
                    + 0.15 * tq              # 交易频率质量（大幅提权，惩罚高频）
                    + exp_pen                # 稀疏惩罚
                    + beta_pen               # Beta 中性惩罚
                    + consist                # 前后一致性奖惩
                )
            return (
                0.40 * ann_ret           # 主目标：年化绝对收益（降权，防高频刷分）
                + 0.20 * port_sortino    # 风险调整辅助（提权）
                + 0.10 * port_calmar     # 回撤控制辅助
                + 0.15 * ts_ic           # IC 预测方向（提权）
                + 0.15 * tq              # 交易频率质量（提权）
                + exp_pen                # 稀疏惩罚
                + beta_pen               # Beta 中性惩罚
                + consist                # 前后一致性奖惩
        ...
        if ModelConfig.REWARD_MODE == "ftmo":
            # FTMO 多品种专属（2026-07-20 调整）：
            #   年化收益 0.75→0.40：降低高频刷分激励
            #   Sortino/Calmar/IC/tq 全面提权：注重质量而非速度
            return (
                0.40 * ann_ret               # 主目标：年化绝对收益（降权）
                + 0.15 * port_sortino        # 风险调整辅助（提权）
                + 0.10 * port_calmar         # 回撤控制（保持，对齐 10% Max Loss）
                + 0.10 * ts_ic               # IC 预测方向（提权）
                + 0.05 * sym_cons            # 品种一致性（保持）
                + 0.05 * cost_s              # 成本压力测试（提权）
                + 0.15 * tq                  # 交易频率质量（大幅提权）
                + exp_pen                    # 稀疏惩罚
                + beta_pen                   # Beta 中性惩罚
                + consist                    # 前后一致性奖惩
