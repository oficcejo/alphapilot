...
class ModelConfig:
    ...
    # ── Actor-Critic（P0-2：启用 critic baseline 替代 EMA 标量）──────────
    # head_critic 一直在模型中定义但未使用。启用后用逐公式 value 估计做
    # baseline，替代全局 EMA 标量，显著降低 REINFORCE 梯度方差。
    # warmup 期间仍用 EMA/batch mean 做 baseline，但 critic loss 始终计算
    # 以训练 critic（避免冷启动问题）。
    USE_ACTOR_CRITIC:   bool  = True
    CRITIC_COEFF:       float = 0.5    # critic MSE loss 权重
