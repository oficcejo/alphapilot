...
**OKX AlphaPilot** 是一个面向 OKX 交易所的独立量化研究平台。它基于强化学习（REINFORCE）+ Looped Transformer 架构，自动搜索由 65 个因果特征和 66 个算子组成的最优因子公式，将因子值转换为 `[-1, 1]` 的连续仓位信号，并通过 StackVM 虚拟机执行、MT5Backtest 多目标评分、Walk-Forward 验证等环节筛选出稳健的 Alpha 策略。
